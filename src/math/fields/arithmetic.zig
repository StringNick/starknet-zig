...
pub fn extendedGCD(comptime T: type, self: T, rhs: T) struct { gcd: T, x: T, y: T } {
    var s = [_]T{ 0, 1 };
    var t = [_]T{ 1, 0 };
    var r = [_]T{ rhs, self };
...
pub fn safeDivBigIntV2(allocator: std.mem.Allocator, x: Int, y: Int) !Int {
    if (y.eqlZero()) return error.DividedByZero;
    ...
    if (!r.eqlZero()) return error.SafeDivFailBigInt;
...
pub fn divModBigInt(allocator: std.mem.Allocator, n: *const Int, m: *const Int, p: *const Int) !Int {
    ...
    if (!igcdex_result.gcd.eql(tmp)) {
        return error.DivModIgcdexNotZero;
    }
...
pub fn isQuadResidue(allocator: std.mem.Allocator, a: Int, p: Int) !bool {
    if (p.eqlZero())
        return error.IsQuadResidueZeroPrime;
