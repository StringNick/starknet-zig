...
pub const ECPoint = struct {
    ...
    pub fn ecAdd(self: *Self, point: ECPoint) ECError!ECPoint {
        ...
        const x = m.powToInt(2).sub(&x_sum);
    ...
    pub fn ecDouble(self: *Self, alpha: Felt252) ECError!ECPoint {
        ...
        const x = m.powToInt(2).sub(&self.x.mul(&Felt252.two()));
        const y = m.mul(&self.x.sub(&x)).sub(&self.y);
        return .{ .x = x, .y = y };
    }
    ...
    pub fn ecDoubleSlope(self: *Self, alpha: Felt252) ECError!Felt252 {
        return try divMod(
            self.x.powToInt(2).mul(&Felt252.three()).add(&alpha),
    ...
    pub fn pointOnCurve(self: *const Self, alpha: Felt252, beta: Felt252) bool {
        const lhs = self.y.powToInt(2);
        const rhs = self.x.powToInt(3).add(&self.x.mul(&alpha).add(&beta));
        return lhs.eql(rhs);
    }
};
